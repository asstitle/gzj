...
class Wxpay extends Controller {
    ...
    public function pay()
    {
        ...
        if($this->request->isPost()) {
            ...
            $member_type = $this->request->param('member_type') ? $this->request->param('member_type') : 0;
            ...
            $fee = $this->request->param('money');//举例支付0.01
    ...
    public function pay_coins(){
        if($this->request->isPost()) {
            ...
            $mch_id = '1517166241';  //商户号
            $nonce_str = $this->nonce_str();//随机字符串
            $notify_url = 'http://gzj.majiangyun.com:80/api/Notify/notify_coins'; //回调的url【自己填写】
            $openid = $openid;
            $out_trade_no = $this->order_number($openid);//商户订单号
            $spbill_create_ip = '121.40.148.132';//服务器的ip【自己填写】;
            $total_fee = $fee*100;// 微信支付单位是分，所以这里需要*100
